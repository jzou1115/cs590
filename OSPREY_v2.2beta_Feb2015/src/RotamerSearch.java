...
public class RotamerSearch implements Serializable
{
	...
	boolean isTemplateOn = false;
        ...
        FBFBSettings fs = new FBFBSettings();//turn off FBFB by defaults
        
        boolean compPairEMatrix = false;//compute minimize total energy for pairs, put in max matrix (HACK)
        EnergyFunction pairEFunc;//energy function for this
	...
	RotamerSearch(Molecule theMolec, int numMut,int strandsPresent, boolean hE, 
			...
			double vdwMult, RotamerLibrary[] rotamerLibraries,
                        ...
                        EPICSettings epics, FBFBSettings fbfbs) {
	...
	public void setAllowable(int resNum, String name, int strNum) {
		strandRot[strNum].setAllowable(resNum,name);
                ...
                //DEBUG!!!
                if(strNum>=m.numberOfStrands){
                    int qq = 0;
                }
                else if(resNum>=m.strand[strNum].numberOfResidues){
                    int qqq = 0;
                }
...
//// BEGIN PAIRWISE MUTATION _ALL_ ROTAMER SEARCH SECTION - ENERGY PRECOMPUTATION
	...
	private double computeEnergyOfOnlyRes(int molResNum, int pos, int[][] strandMut, Amber96ext resA96ff) {
            
		if(resA96ff == null) //create the template amber FF if it's null
                    resA96ff = initOnlyResEnergy(molResNum, pos, resA96ff);
				
		double energyTerms[] = resA96ff.calculateTotalEnergy(m.actualCoordinates,-1); //compute the energy
		double curEnergy = (double) energyTerms[0];
			
		return curEnergy;
	}

        private Amber96ext initOnlyResEnergy(int molResNum, int pos, Amber96ext resA96ff){
	...
	//  residue of interest have been computed prior to calling
	//  this function as they are not computed in this function
	private double computeEnergyOfOnlyTemplate(int strandMut[][]) {
		if(templateA96ff == null) //create the template amber FF if it's null
			initTemplateEnergy(strandMut);
        ...
        private void initTemplateEnergy(int strandMut[][]){
        ...
        private Amber96ext fullTupleFF(int strandMut[][], int tuple[]){
            //generate an Amber96ext for the specified tuple of positions (numbered among mutable)
            //including template

            Amber96ext ans = new Amber96ext(m, distDepDielect, dielectConst, doSolvationE, solvScale, vdwMultiplier);

            boolean savedEnergyEvalSC[] = new boolean[m.numberOfResidues];
            boolean savedEnergyEvalBB[] = new boolean[m.numberOfResidues];


            // Save the energy eval flag, turn everything on at the same time
            for(int i=0;i<m.numberOfResidues;i++){
                    savedEnergyEvalSC[i] = m.residue[i].getEnergyEvalSC();
                    savedEnergyEvalBB[i] = m.residue[i].getEnergyEvalBB();
                    m.residue[i].setEnergyEval(true, true);
            ...
            //Clear the energy evaluation flags for the AS residues and the ligand (if present)
            //turn on only if in specified tuple
            int posCount = 0;
            ...
            for (int str=0; str<strandMut.length;str++){
                    for (int i=0; i<strandMut[str].length; i++){
                        ...
                        if(inTuple)
                            m.strand[str].residue[strandMut[str][i]].setEnergyEval(true, true);
                        else
                            m.strand[str].residue[strandMut[str][i]].setEnergyEval(false, false);
            ...
            ans.initializeCalculation();
            ans.setNBEval(hElect,hVDW);
	...
	public void simplePairwiseMutationAllRotamerSearch(int strandMut[][], int mutableSpots, 
		...
		computeEVEnergy = true;
                ...
                if(useCCD){//Need a list of degrees of freedom to use the CCD minimizer
                    //we may also need to initialize FBFB flexibility
                    m.fbfb = fs.makeFBFB(m);
                    m.DOFs = DegreeOfFreedom.makeDOFArray(strandRot, strandMut, mutRes2Strand, mutRes2StrandMutIndex, m, strandDefault,fs);
                }
		...
		if(computeEVEnergy){
			...
			if (templateOnly) { //the template energies are computed only once	
				...
				if (!minimizeBB) {//side-chain minimization, so the template is fixed	
					...
					double beginE = computeEnergyOfOnlyTemplate(strandMut);//calcTotalSnapshotEnergy();
					simpMin.minimize(35);
					double curE = computeEnergyOfOnlyTemplate(strandMut); //calcTotalSnapshotEnergy();
                                ...
                                if (!minimizeBB){ //side-chain minimization
                                        if(useCCD){
                                            ...
                                            if(minimizePairwise){
                                                ...
                                                //Set up energy function to minimize
                                                EnergyFunction mainEF[] = { new ForceFieldEnergy(m, templateOffA96ff) };
                                                EnergyFunction negEFs[] = null;

                                                if(shellRun){//Need to subtract off template energy
                                                    //initialize templateA96ff
                                                    initTemplateEnergy(strandMut);
                                                    negEFs = new EnergyFunction[] { new ForceFieldEnergy(m, templateA96ff) };
                                                }

                                                else{//Need to subtract off intra energies
                                                    //Initialize res1A96ff, res2A96ff
                                                    int res1Strand = mutRes2Strand[res1];
                                                    int res1Resnum = strandMut[res1Strand][mutRes2StrandMutIndex[res1]];
                                                    int res2Strand = mutRes2Strand[res2];
                                                    int res2Resnum = strandMut[res2Strand][mutRes2StrandMutIndex[res2]];

                                                    initOnlyResEnergy(m.strand[res1Strand].residue[res1Resnum].moleculeResidueNumber,0,res1A96ff);
                                                    initOnlyResEnergy(m.strand[res2Strand].residue[res2Resnum].moleculeResidueNumber,1,res2A96ff);

                                                    negEFs = new EnergyFunction[] { new ForceFieldEnergy(m, res1A96ff) ,
                                                        new ForceFieldEnergy(m, res2A96ff) };
                                                    
                                                
                                                    if(compPairEMatrix){
                                                        initTemplateEnergy(strandMut);
                                                        Amber96ext pairFF = fullTupleFF(strandMut, new int[] {res1,res2});
                                                        pairEFunc = new MultiTermEnergyFunction(
                                                                new ForceFieldEnergy[] {new ForceFieldEnergy(m, pairFF)},
                                                                new ForceFieldEnergy[] {new ForceFieldEnergy(m, templateA96ff)}
                                                                );
                                                    }
                                                }

                                                //ef = mainEF - negEFs
                                                efunc = new MultiTermEnergyFunction(mainEF,negEFs);
                                            }
                                            else
...
//validConf indicates that all RC applications were successful (always true if not DEEPer)
				...
				if ((stericallyGood)) { //good steric found or doing backbone minimization
					...
					if (doMinimization){
						...
						if(!shellRun){
							...
							double tmpE = computeEnergyOfOnlyRes(m.strand[res1Strand].residue[res1Resnum].moleculeResidueNumber,0,strandMut,res1A96ff);
							curEnergy -= tmpE;
	...
	private void pairwiseRotamerEnergySidechainHelper (int res1,int res2,int res1Strand,int res1Resnum,int res2Strand,int res2Resnum,int strandMut[][],
		...
		double beginE = 0.0f;
                ...
                if(minimizePairwise){//ef will be the total energy we are minimizing (e.g. pairwise = pair energy - intra energies)
                    ...
                    if(compPairEMatrix && !shellRun){
                        //minimize full pair energy
                        
                        boolean[] trs = ((ContSCObjFunction)ccdMin.objFcn).transRotStrands;
                        ContSCObjFunction pairOF = 
                                new ContSCObjFunction(m,numberOfStrands,pairEFunc,strandRot,curAANum,false,trs);
                        
                        CCDMinimizer pairMin = new CCDMinimizer(pairOF,true);
                        //CCDMinimizer pairMin = new CCDMinimizer(pairOF,false);//this should agree with overall min for 2-res design
                        
                        if(doDihedE)
                            throw new RuntimeException("dihedrals not supported here...adjust above");
                        
                        m.updateCoordinates();//restore the actualCoordinates array to the initial values
                        m.revertPertParamsToCurState();
                        pairMin.minimize();//re-minimize for pair energy
                        double pairEnergy = (double)pairEFunc.getEnergy();
                        
                        //we'll stick this in retEMatrixMax
                        retEMatrixMax.setPairwiseE( res1, res1AANum, res1RotNum, res2, res2AANum, res2RotNum, pairEnergy );
                    }
                    ...
                    if (!shellRun) {
                            //Remove the intra-residue energies here
                            double tmpE1 = computeEnergyOfOnlyRes(m.strand[res1Strand].residue[res1Resnum].moleculeResidueNumber,0,strandMut,res1A96ff);
                            beginE -= tmpE1;
                    ...
                    if (!shellRun) {
                            //Remove the intra-residue energies here
                            double tmpE2 = computeEnergyOfOnlyRes(m.strand[res1Strand].residue[res1Resnum].moleculeResidueNumber,0,strandMut,res1A96ff);
                            curEnergy -= tmpE2;
        ...
        private void compEPICFit(int str1, int strResNum1, int res1, int res1AANum, int res1RotNum, int str2,
            ...
            if(ccdMin.numDOFs>0){//need to actually compute polynomial fits
                ...
                if(es.calcNCEllipses)
                    fitter.makeNCEllipse(bestSer);
            }
            else
            ...
            //fitter.makeVoxelFigureData(bestSer);
	...
	private double removeExtraE(boolean templateOnly, int res1Strand, int res1Resnum, int res2Strand, int res2Resnum, int strandMut[][]){
		...
		if (eliminatedRotAtRes == null){
			...
			return;
                ...
                if(es.useEPIC)//For EPIC we will refer to DOFNums in the CETMatrix, so DOFNums must match cetm.DOFList
                    ...
                    m.fbfb = fs.makeFBFB(m);
                    m.DOFs = DegreeOfFreedom.makeDOFArray(strandRot, strandMut, mutRes2Strand, mutRes2StrandMutIndex, m, null,fs);
                }
 ...
 */		
	...
	public PrunedRotamers<Boolean> DoDEEGoldstein(int numMutable,
		...
		DEEGoldstein DEERun = new DEEGoldstein(arpMatrix, arpMatrixMax, numMutable, strandMut, initEw, 
				...
				typeDep, useMinDEEPruningEw, Ival, doPerturbations, true);
	...
	public void DoDEEPairs(int numMutable, int strandMut[][], 
		...
		DEEGoldsteinPairs DEERunPairs = new DEEGoldsteinPairs(arpMatrix, arpMatrixMax, numMutable, strandMut, initEw, strandRot, prunedRotAtRes, resInPair, doMinimize, 
				...
				mutRes2Strand, mutRes2StrandMutIndex, typeDep, doIMinDEE, Ival, tripleFlags, doPerturbations,true);
	...
	private double doAStarGMECHelper(int numMutable, int strandMut[][], String fileName, 
		...
		if (outputFile){
			try {			
              ...
              //DEBUG!!!  
            /*for(ContETerm cet : cetm.pairwiseBounds[1][17][5][0][9]){
                if(cet!=null){
                    EPoly ep = (EPoly)cet;
                    if(ep.sve==null){
                        DoubleMatrix1D x = ep.DOFmin.copy().assign(DoubleFactory1D.dense.random(ep.numDOFs),Functions.plus);
                        System.out.println(ep.evaluate(x, true, false));
                        ep.recenter(DoubleFactory1D.dense.random(ep.numDOFs));
                        System.out.println(ep.evaluate(x, true, false));
                        System.out.println();
                    }
                }
            ...
            //DEBUG!!! Trying out ellipse-based ClashFilter
            /*ClashFilter.useEllipses = true;
            boolean nc = new ClashFilter(cetm,new int[] {9,17},0).hasNonClashing();
            int aaa = 1;*/
            
                
            //DEBUG!!! Trying out FullBBFlexBlock
            /*FullBBFlexBlock fbfb = new FullBBFlexBlock(m,36,41,false);
            try {
                new KSParser().saveMolecule( (Molecule)KSParser.deepCopy(m), "FBFBinit.pdb", 0);
                
                
                for(int dof=0; dof<6; dof++){//try little motions in every direction
                    DoubleMatrix1D x = DoubleFactory1D.dense.make(6);
                    x.set(dof,1);
                    boolean closed = fbfb.setFreeDOFs(x);
                    DoubleMatrix1D xCheck = fbfb.recordFreeDOFs();
                    if(closed)
                        new KSParser().saveMolecule( (Molecule)KSParser.deepCopy(m), "FBFB"+dof+".pdb", 0);
                }
            }
            catch(Exception e){
            ...
            //DEBUG!!!!!!!  Trying out NWisePolyFitter
            /*if(!es.gettingLowestBound){
                a96ff.calculateTypesWithTemplates();
                a96ff.initializeCalculation();
                a96ff.setNBEval(hElect,hVDW);
                efunc = new ForceFieldEnergy(m, a96ff);
                //assuming no dihedral energies for now
                
                
                //DEBUG!!!!
                /*NWisePolyFitter nwpf = (NWisePolyFitter)KSParser.readObject("NWPF1396114483035.dat");
                //RELOADING CLASHES SINCE CLASH IS STATIC
                for(int p=0; p<nwpf.polyParams.size(); p++){
                    if(nwpf.polyParams.get(p).order==0)//clashes
                        nwpf.polyParams.set(p, FitParams.CLASH);
                }*/
                
                /*NWisePolyFitter nwpf = new NWisePolyFitter(numMutable, m.DOFs, efunc, m, strandRot, strandMut, mutRes2Strand, mutRes2StrandMutIndex, false);
                nwpf.prunedRot = eliminatedRotAtRes;
                nwpf.splitFlags = splitFlags;
                nwpf.initCETM = cetm;
                
                nwpf.doFits();
                nwpf.loadCETM(cetm,arpMatrix);
                //KSParser.outputObject(cetm, "NWPFCETM.dat");
                //KSParser.outputObject(arpMatrix, "NWPFPEM.dat");
                
                //cetm = (CETMatrix)KSParser.readObject("NWPFCETM.dat");
                //arpMatrix = (PairwiseEnergyMatrix)KSParser.readObject("NWPFPEM.dat");
                
                arpMatrixRed = arpMatrix.reduceMatrix( eliminatedRotAtPosRed,
			numRotForRes, numRotForResNonPruned, treeLevels,
			numTotalRotRedNonPruned, numMutable, strandMut,
                        numTotalRotRed, this, false, splitFlagsRed, tripleFlagsRed);
            }*/
            //DEBUG!!!
                ...
                /*
                CETMatrix cetm2 = cetm;
                cetm = (CETMatrix)KSParser.readObject("NWPFCETM.dat");
            
            
            
            for(int rot=0; rot<=17; rot++)
                if(cetm.intraAndShellBounds[0][9][rot]!=null)
                    arpMatrix.setShellRotE(0, 9, rot, cetm.intraAndShellBounds[0][9][rot].minE);
            
            for(int rot=0; rot<=7; rot++){
                if(cetm.intraAndShellBounds[1][17][rot]!=null)
                    arpMatrix.setShellRotE(1, 17, rot, cetm.intraAndShellBounds[1][17][rot].minE);
                for(int rot2=0; rot2<=17; rot2++)
                    if(cetm.pairwiseBounds[1][17][rot][0][9][rot2]!=null)
                        arpMatrix.setPairwiseE(1, 17, rot, 0, 9, rot2, cetm.pairwiseBounds[1][17][rot][0][9][rot2].minE);
            }
            ...
            */
            ...
            //trying out ConfETupeExpander
            //ok the idea is, we will create and save a TupleEnergyMatrix (extends Pairwise"") here
            //then we'll rerun w/ rigid
            //of course later we may be better off working on ConfETupExpander interlaced w/ pruning, 
            //even A* in order to avoid having to go to high order on unpromising tuples
                
                //starting with just computing tuple expansion!
          
                /*
            boolean useG = false;
            ConfETupExpander cete = new ConfETupExpander(this,strandMut,arpMatrixRed,
                    numRotForResNonPruned,true,useG);
            cete.doFits();
            System.exit(0);
              */
                ...
                //DEBUG!!!
                /*ConfTree ct = new ConfTree(arpMatrixRed,arpMatrix);
                ct.splitFlags = splitFlags;
                ct.tripleFlags = tripleFlags;
                ct.cetm = cetm;*/
		...
		while (true){
			...
			//curLigRotNum = -1;
                        ...
                        //DEBUG!!!!
                        //int ctConf[] = ct.doAStar(run1, numMutable, AAdefault, eliminatedRotAtPosRed, strandRot, strandDefault, numRotForRes, strandMut, minimizeBB, mutRes2Strand, mutRes2Strand);
			...
			boolean done = false;
                        ...
                        if(es.useEPIC){
                            ...
                            //DEBUG!!
                            //new VoxPartFcn(lof).checkG();
                            /*VoxPartFcn vpf = new VoxPartFcn(lof);
                            long gStartTime = System.currentTimeMillis();
                            System.out.println( "FREE ENERGY: "+vpf.calcGVEGAS()+" "+vpf.calcGVEGAS()+" "+vpf.calcGVEGAS()+
                                    " Time (ms): "+(System.currentTimeMillis()-gStartTime) );
                            */
                                    ...
                                    if (doMinimization){
                                            if (!minimizeBB) {//side-chain minimization
                                                    ...
                                                    if(useCCD){//The ideal dihedrals are already recorded since we just reinitialized ccdMin
                                                        ...
                                                        //DEBUG!!!
                                                        //new KSParser().saveMolecule(m, "CHECKUNMIN.pdb", unMinE);
                                                        ...
                                                        //DEBUG!!!
                                                        //new KSParser().saveMolecule(m, "CHECKMIN.pdb", minE);
                                                        ...
                                                        //DEBUG!!!!!
                                                        //new VoxPartFcn(ccdMin.objFcn).checkG();
