...
public class ContSCObjFunction implements ObjectiveFunction, Serializable {
	...
	//  used in minimization
        ...
        boolean fbfbValid[];//if there are FullBBFlexBlocks, do they have valid loop closure?
    ...
    public ContSCObjFunction(Molecule theM, int numStrands, EnergyFunction ef,
		...
		if (doDihedEnergy){//This assumes ef.getAmber96ext() is well defined (since dihedral energies are made to be used with force field energies).  There will be an error otherwise
                ...
                calcDOFNums(m.DOFs);//Fill in DOFNums, numDOFs, flexToMolResMap, fbfb
                ...
                fbfbValid = new boolean[m.fbfb.length];
                Arrays.fill(fbfbValid,true);//we start with a closed mainchain
    }
    ...
    private void calcDOFNums(DegreeOfFreedom[] DOFList){
        ...
        for(FullBBFlexBlock curFBFB : m.fbfb)
            numDOFs += curFBFB.numFreeDOFs;
        ...
        for( DegreeOfFreedom curDOF : DOFList ){
            ...
            else if(curDOF.type == DegreeOfFreedom.FBFBPARAM){
                //let's say these can all be minimized over
                //we'll be making alternate DOFs for pairwise minimizations soon sanyway
                DOFNums[DOFCount] = curDOF.DOFNum;
                DOFCount++;
            }
        }
    ...
    private void setupPartialComp(){
        ...
        for(int dof=0; dof<numDOFs; dof++){
             ...
             */
                ...
                //any residues in the FBFB count toward the partial sets
                //of course if we are doing pairwise energy some of these may not have energies counted based on evalEnergy{SC,BB}
                FullBBFlexBlock curFBFB = m.fbfb[curDOF.fbfbNum];
                partialSets[dof+1] = curFBFB.paramAffectedRes(curDOF.fbfbParamNum);
                
                for(int res : partialSets[dof+1])
                    allAffectedRes.add(res);
            }
        }
...
/*    public int getNumDOFs(){
    ...
    public DoubleMatrix1D[] getConstraints(){
        ...
        for(int dof=0; dof<numDOFs; dof++){//Loop over minimizing DOFs
            ...
            if( specialBoxConstraints[dof] == null ){//Default constraints
                ...
                if(curDOF.type == DegreeOfFreedom.SCDIHEDRAL){
                ...
                else if(curDOF.type == DegreeOfFreedom.STRRIGIDMOTION || curDOF.type==DegreeOfFreedom.FBFBPARAM){
                    ...
                    if(curDOF.intervals.length>1)
                        throw new RuntimeException("ERROR: DOF "+dof+" of type "+curDOF.type+
                                " expected to have just 1 interval, has "+curDOF.intervals.length);
                }
                else{
    ...
    boolean allFBFBValid(){
        //do all FBFBs have valid loop closure?
        for(boolean b : fbfbValid){
            if(!b)
                return false;
        }
        return true;
    ...
    public void setDOFs(DoubleMatrix1D x){
        ...
        //FBFB params for each block are set together, before setting other variables
        for(int f=0; f<m.fbfb.length; f++){
            
            FullBBFlexBlock fb = m.fbfb[f];
            DoubleMatrix1D fbfbInput = DoubleFactory1D.dense.make(fb.numFreeDOFs);
            
            for(int dof=0; dof<numDOFs; dof++){
                DegreeOfFreedom curDOF = m.DOFs[DOFNums[dof]];
                if( curDOF.type==DegreeOfFreedom.FBFBPARAM ){
                    if(curDOF.fbfbNum==f)
                        fbfbInput.set(curDOF.fbfbParamNum, x.get(dof));
                }
            }
            
            if(fb.setFreeDOFs(fbfbInput))
                fbfbValid[f] = true;
            else
                fbfbValid[f] = false;
        }
        ...
        for(int dof=0; dof<numDOFs; dof++){
            if( m.DOFs[DOFNums[dof]].type != DegreeOfFreedom.FBFBPARAM )
    ...
    public void setDOF(int dof, double val){
        ...
        if(curDOF.type == DegreeOfFreedom.SCDIHEDRAL){
            ...
            FullBBFlexBlock fb = m.fbfb[curDOF.fbfbNum];
            DoubleMatrix1D fbfbInput = DoubleFactory1D.dense.make(fb.numFreeDOFs);
            
            for(int d=0; d<numDOFs; d++){
                DegreeOfFreedom thisDOF = m.DOFs[DOFNums[d]];
                if( thisDOF.type==DegreeOfFreedom.FBFBPARAM ){
                    if(thisDOF.fbfbNum==curDOF.fbfbNum)
                        fbfbInput.set(thisDOF.fbfbParamNum, curDOFVals.get(d));
                }
            }
            
            if(fb.setFreeDOFs(fbfbInput))
                fbfbValid[curDOF.fbfbNum] = true;
            else
                fbfbValid[curDOF.fbfbNum] = false;
