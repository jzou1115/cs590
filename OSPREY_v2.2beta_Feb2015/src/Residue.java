...
public class Residue implements Serializable {
	...
	public Atom getAtomByName(String n){
     ...
     // Returns true if r2_pdbResNum precedes r1 in the PDB chain. (e.g. residue 19 precedes residue 20g)
        ...
        public void shiftPDBResNumber(int offset){
            //add offset to the integer part of the PDB residue number
            
            int curResNumber = getResNumIntegerPart( getResNumberString() );
            String curKabatLetter = getResNumKabatLetter( getResNumberString() );
            String newResNumber = String.valueOf( curResNumber + offset );
            
            String newFullName = fullName.substring(0,5);
            
            for(int q=0; q<4-newResNumber.length(); q++)//4 columns allocated for number
                newFullName = newFullName + " ";
            newFullName = newFullName + newResNumber;
            
            if(curKabatLetter.length()==0)
                newFullName = newFullName;
            else
                newFullName = newFullName + curKabatLetter;
