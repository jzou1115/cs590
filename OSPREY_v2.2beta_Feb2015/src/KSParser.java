...
public class KSParser
{
	 ...
	 * 
	 */
	public void checkMPI(String[] args) throws IOException {
	...
	*/
	public void parse(String[] args) throws IOException {
		...
		/*else if (firstToken.equalsIgnoreCase("doSinglePartFn"))
			...
			fixStruct(s);
                else if (firstToken.equalsIgnoreCase("doMultistateAStar"))
			handleDoMultistateAStar(s);
	...
	*/
	public void outputProgInfo() {
		...
		System.out.println();
		System.out.println("OSPREY Protein Redesign Software Version 2.2 beta");
		System.out.println("Copyright (C) 2001-2014 Bruce Donald Lab, Duke University");
		System.out.println("");
	...
	fileOutputStream);
			...
			System.exit(0);
                        ...
                        if(e instanceof RuntimeException)
                            throw (RuntimeException)e;//so we can see what happened
	 ...
	 * @throws IOException 
	*/
	public void handleKSMaster(String s) throws IOException {
		...
		boolean selectPerturbations = (new Boolean((String)sParams.getValue("SELECTPERTURBATIONS","false"))).booleanValue();//Should perturbations be automatically selected?
                ...
                FBFBSettings fs = new FBFBSettings(sParams);
		...
		RotamerSearch rs = new RotamerSearch(m,numberMutable, strandsPresent,hElect,hVDW,hSteric,true,true,
                        ...
                        softvdwMultiplier,grl, doPerturbations, pertFile, minimizePerts, false, false, es, fs);
		...
		if (mutSet == null) {
			...
			RotamerSearch rs = new RotamerSearch(m, numMutable, strandsPresent, hElect, hVDW, hSteric, true,
					true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst,doDihedE,
                                        ...
                                        false,null,false,false,false,new EPICSettings(),new FBFBSettings());//Not going to consider perturbations, etc. at this level of approximation
	...
	*/
	...
	public CommucObj handleKSSlave(CommucObj cObj) throws IOException {
	 ...
	 * @throws IOException 
	 */
	private CommucObj hybridKScompute(CommucObj cObj) throws IOException{
		...
		System.out.println(" &&");
                ...
                if( Boolean.valueOf(cObj.params.getValue("USECLASHFILTER","false")) ) {
                    ...
                    if(doClashFilter(cObj)){//sequence pruned by clash filter
                        //fill in partition functions indicating no binding
                        for(int i=0; i<cObj.numComplexes-1;i++)
                            cObj.q[i] = new BigDecimal("1.0");
                        cObj.q[cObj.numComplexes-1] = new BigDecimal("0.0");
		...
		for(int runNum = 0; runNum<allCombos.length; runNum++) {
            ...
            //removing m and grl storage for now because it messes up both DEEPer and multistate A*
        ...
        boolean doClashFilter(CommucObj cObj){
            //demand that desired ligand has >0 non-clashing confs,
            //and if altCETMName isn't None, to have alternate ligand clash in all confs.  
            //if demands are not met then prune the sequences (by returning true)
            ...
            String altCETMName = cObj.params.getValue("ALTCETMNAME","None");
            ...
            int curSeq[] = new int[thisCETM.numRes];
            
            for(int res=0; res<thisCETM.numRes; res++){
                
                curSeq[res] = -1;
                for(int index=0; index<thisCETM.resAATypes[res].length; index++){
                    if( thisCETM.resAATypes[res][index].equalsIgnoreCase(cObj.currentMutation[res]) ){
                        curSeq[res] = index;
                        break;
                    }
                }
            ...
            ClashFilter.useEllipses = Boolean.valueOf(cObj.params.getValue("CLASHFILTERUSEELLIPSES","true"));
            ...
            //note: with ellipse-based clash filter we instead filter out regions where at least one term is in an unfavorable region
            //EPICThresh1 is used for this
            if(ClashFilter.useEllipses && !cObj.params.getValue("CLASHFILTERCUTOFF","NAN").equalsIgnoreCase("NAN"))
                throw new RuntimeException("ERROR: Clash filter with ellipses must use EPICThresh1 as cutoff (for each EPIC term)!");
            
            //perform actual clash filtering for this system
            ...
            if(!altCETMName.equalsIgnoreCase("None")){//also want to require clashes for alternate system
	...
	private void loadPairwiseEnergyMatrices(ParamSet sParams, RotamerSearch rs, String minMatrixFile, boolean doMinimize, String maxMatrixFile,
										int strandPresent) throws IOException{
        ...
        private void loadCETMatrix( ParamSet sParams, RotamerSearch rs, int strandPresent, double ival, boolean local ) throws IOException{
                String runName = sParams.getValue("RUNNAME");
		String suffix = "_" + strandPresent+".dat";
		if(strandPresent == COMPLEX )
			suffix = "_COM.dat";
        ...
        private void loadUnboundCETMatrix( ParamSet sParams, RotamerSearch rs, double ival, boolean local, int unboundStrand ) throws IOException{
	...
	private void loadUnboundPairwiseEnergyMatrices(ParamSet sParams, RotamerSearch rs, String minMatrixFile, boolean doMinimize, String maxMatrixFile,
										int unboundStrand) throws IOException{
		//StrandNumbers are appended to PEM matrix, Complex is denoted with "_COM"
          // PGC
                String runName = sParams.getValue("RUNNAME");
...
/////////////////////////////////////////////////////////////////////////
    ...
    public void handleComputeAllPairwiseRotamerEnergies(String s) throws IOException{
	...
	public void handleComputeAllPairwiseRotamerEnergiesMaster(ParamSet sParams, boolean compCETM, PrunedRotamers<Boolean> prunedRot, boolean[][][][][][] splitFlags, double ival, 
                boolean local, EPICSettings es ) throws IOException {
		...
		RotamerSearch rs = new RotamerSearch(m, numberMutable, strandsPresent, hElect, hVDW, hSteric, true,
				true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst, doDihedE, 
                                doSolvationE, solvScale, softvdwMultiplier, grl, doPerturbations, pertFile, minimizePerts, 
                                false, false, es, fs);
		...
		mutMan.setcurStrForMatrix(curStrForMatrix);
                ...
                if(compCETM){
                    ...
                    m.DOFs = DegreeOfFreedom.makeDOFArray(rs.strandRot, strandMut,
                            rs.mutRes2Strand, rs.mutRes2StrandMutIndex, m, strandDefault, fs);
        ...
        mutationEnergiesMin.write("mutationEnergiesMin.txt");
        mutationEnergiesMax.write("mutationEnergiesMax.txt");
		System.out.println("DONE: Pairwise energy matrix precomputation..");
	}
...
///////////////////////////////////////////////////////////////////////////
	 ...
	 * @throws IOException 
	*/
	public void handleDoDEE(String s) throws IOException {
		...
		do{
			...
			RotamerSearch rs = new RotamerSearch(mp.m,mp.numberMutable, mp.strandsPresent, hElect, hVDW, hSteric, true,
					true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst, doDihedE, doSolvationE, solvScale, softvdwMultiplier, grl,
                                        doPerturbations, pertFile, minimizePerts, useTriples, useFlagsAStar, es, fs);
			...
			long pruneTime = System.currentTimeMillis();
                        ...
                        if(setupState>=0){//we're setting up for multistate A*
                            //stop this single-state calc here and do multistate A*
                            System.out.println("Finished setup for state "+setupState);
                            System.out.println("Pruning time: "+((pruneTime-startTime)/(60.0*1000.0)));
                            if(setupNegState){
                                System.out.println("(Negative version)");
                                negStateRS[setupState] = rs;
                            }
                            else {
                                stateRS[setupState] = rs;
                                stateStrandMut[setupState] = mp.strandMut;
                            }
                            return;
                        }
        ...
        //some variables to keep track of stuff for multistate A*
        int setupState = -1;//if we're setting up a state for multistate A* using handleDoDEE, 
        //write state # here (keep -1 if not)
        RotamerSearch stateRS[] = null;
        int[][][] stateStrandMut = null;
        
        boolean setupNegState = false;//setting up special negative-coefficient version of state
        RotamerSearch negStateRS[] = null;//the RotamerSearch objects for this
        
        
        
        public void handleDoMultistateAStar(String s) throws IOException {
            //To do multistate A*, we'll take a Multistate.cfg main configuration file
            //this will point to System.cfg and DEE.cfg files for each state,
            //which should match in terms of relevant details and are written just
            //as if we just wanted GMECs for the states
            
            long startTimeAll = System.currentTimeMillis();
            
            System.out.println("Performing multistate A*");
            ParamSet sParams = new ParamSet();
            sParams.addParamsFromFile(getToken(s,2)); //read multistate parameters
            
            
            //handle state-specific setup: parsing params, energy matrix precomp, etc.
            int numStates = (new Integer((String)sParams.getValue("NUMSTATES"))).intValue();
            
            stateRS = new RotamerSearch[numStates];
            negStateRS = new RotamerSearch[numStates];
            stateStrandMut = new int[numStates][][];
            
            String defaultCFGName = cfgName;//this is the KStar.cfg to use unless specified otherwise
            //in the StateKSFile fields
            
            for(int state=0; state<numStates; state++){
                
                //set up config parameters based on correct KStar.cfg for state
                cfgName = (String)sParams.getValue("STATEKSFILE"+state,"DEFAULT");
                if(cfgName.equalsIgnoreCase("DEFAULT"))
                    cfgName = defaultCFGName;
                
                setConfigPars();//sets appropriate energy function and rot lib parameters for state
                //these will then be either stored in rs or used to compute E/EPIC matrices & not needed further
                //even if KStar.cfg stays the same, this reloads the rotamer library
                //so if we're adding wt rots to the rotamer library this is done correctly for each state
                
                //generate a fake DEE command to induce setup of the state.  We expect input like
                //STATECFGFILES0 System0.cfg DEE0.cfg
                setupState = state;
                String fakeDEECommand = "doDEE "+(String)sParams.getValue("STATECFGFILES"+state);
                handleDoDEE(fakeDEECommand);
                
                //need to ensure type-dep!
                                
                //negative setup is useful, for example, for cont flex
                //where we want a negative version of each state, like
                //the normal version but rigid and with a different run name
                //to provide lower bounds for partial sequences
                //where cont flex is bounded using discrete approx              
                String negCommand = (String)sParams.getValue("NEGSTATECFGFILES"+state,"None");
                if(negCommand.equalsIgnoreCase("None"))//None means just use normal version
                    negStateRS[state] = stateRS[state];
                else {
                    
                    cfgName = (String)sParams.getValue("STATENEGKSFILE"+state,"DEFAULT");
                    if(cfgName.equalsIgnoreCase("DEFAULT"))
                        cfgName = defaultCFGName;
                    ...
                    setupNegState = true;
                    fakeDEECommand = "doDEE "+negCommand;
                    handleDoDEE(fakeDEECommand);
                    setupNegState = false;
                }
            }
            ...
            System.out.println("Setup complete...time for A*");
            
            int numSeqsWanted = (new Integer((String)sParams.getValue("NUMSEQS","1"))).intValue();
            //how many sequences to enumerate
                        
            long startAStarTime = System.currentTimeMillis();

            SeqTree stree = new SeqTree(stateRS,negStateRS,stateStrandMut,sParams);
            
            for(int seqNum=0; seqNum<numSeqsWanted; seqNum++){
                int seq[] = stree.doAStar();//this will find the best sequence and print it
                if(seq[0]==-1)//empty sequence...indicates no more sequence possibilities
                    break;
            }
	...
	private CommucObj doDistrDEESlave(CommucObj cObj) {
		...
		RotamerSearch rs = new RotamerSearch(m,cObj.mutableSpots,cObj.strandsPresent, hElect, hVDW, hSteric, true,
					true, cObj.epsilon, cObj.stericThresh, cObj.softStericThresh, cObj.distDepDielect, 
                                        ...
                                        new EPICSettings(), new FBFBSettings(cObj.params));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void handleDoMPI(String args[]) throws MPIException, InterruptedException, IOException {
	...
	public void handleDoMPIMaster(MutationManager mutMan, int size) throws MPIException, InterruptedException {
        ...
        public void handleMasterLocally(MutationManager mutMan, int size) throws IOException {
	...
	public void handleDoMPISlave() throws MPIException, InterruptedException, IOException {
	...
	public void generateBackbones(String s){
		...
		RotamerSearch rs = new RotamerSearch(mp.m,numberMutable,strandsPresent, hElect, hVDW, hSteric, true,
					true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst, doDihedE, 
                                        ...
                                        new EPICSettings(), new FBFBSettings());
	...
	public void doSinglePairE(String s, ParamSet sParams) {
		...
		RotamerSearch rs = new RotamerSearch(m,numberMutable,strandsPresent, hElect, hVDW, hSteric, true,
					true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst,
                                        ...
                                        false, null, false, false, false, new EPICSettings(), new FBFBSettings(sParams));
	...
	private CommucObj handleDoResEntropySlave(CommucObj cObj){
		...
		if (cObj.compASdist){ //AS-AS distance computation
			...
			RotamerSearch rs = new RotamerSearch(m, cObj.mutableSpots, strandsPresent, hElect, hVDW, hSteric, true,
						true, cObj.epsilon, cObj.stericThresh, cObj.softStericThresh, cObj.distDepDielect, 
                                                cObj.dielectConst, cObj.doDihedE,cObj.doSolvationE,cObj.solvScale,cObj.vdwMult, grl,
                                                false, null, false, false, false, new EPICSettings(), new FBFBSettings(cObj.params));
