...
public class DegreeOfFreedom implements RyanComparable, Serializable {//DOF for short
    ...
    static int FBFBPARAM = 5;//FullBBFlexBlock parameter
    ...
    //we'll need these for FBFB too
    double FBFBMeshWidth;
    boolean isFBFBParamAngle;
    ...
    //if this is an FBFBPARAM we need the block (numbered within the molecule's blocks) and the parameter numbers
    int fbfbNum = 0;
    int fbfbParamNum = 0;
    ...
    public DegreeOfFreedom(int DOFNumber, int rigidDOFNumber, int strandNumber, 
            int strandMut[][], int mutRes2Strand[], int mutRes2StrandMutIndex[], StrandRotamers strandRot){
        ...
        for(int ares=0; ares<numResAffected; ares++){//Loop through affected residues
            ...
            int curStrResNum = strandMut[strandNum][mutRes2StrandMutIndex[flexResAffected[ares]]];
            
            int numAllowable = strandRot.getNumAllowable(curStrResNum);

            for(int AA=0; AA<numAllowable; AA++){
                int AAIndex = strandRot.getIndexOfNthAllowable(curStrResNum, AA);
                int numRCs = getNumRCs(strandRot, AAIndex);
    ...
    //Constructor for FBFB params
    public DegreeOfFreedom(int DOFNumber, Molecule m, int FBNum, int paramNum, FBFBSettings fs, 
            int molToFlexResNumMap[], StrandRotamers strandRots[]){

        type=FBFBPARAM;
        DOFNum = DOFNumber;
        
        fbfbNum = FBNum;
        fbfbParamNum = paramNum;

        FullBBFlexBlock fbfb = m.fbfb[fbfbNum];
        FBFBMeshWidth = fbfb.getMeshWidth(paramNum);
        isFBFBParamAngle = fbfb.isParamAngle(paramNum);
        
        
        int numResAffected = fbfb.endRes-fbfb.startRes+1;
        flexResAffected = new int[numResAffected];
        for(int a=0; a<numResAffected; a++)
            flexResAffected[a] = molToFlexResNumMap[fbfb.startRes+a];
        

        //For now, we will only have one interval for these motions
        numIntervals = 1;
        
        if(fbfb.freeDOFs[paramNum].type==FullBBFlexBlock.FIRSTCAADJ)//translation
            intervals = new double[][] { new double[] { -fs.maxTrans, fs.maxTrans } };
        else if(fbfb.freeDOFs[paramNum].type==FullBBFlexBlock.ANCHORDIH){//rotation
            double origDihedral = fbfb.measureAnchorDih(fbfb.freeDOFs[paramNum].anchorDihNum, false);
            intervals = new double[][] { new double[] { origDihedral-fs.maxDihChange, origDihedral+fs.maxDihChange } };
        }
        else//rotation
            intervals = new double[][] { new double[] { -fs.maxRot, fs.maxRot } };

        StrandRotamers strandRot = strandRots[m.residue[fbfb.startRes].strandNumber];
        //the whole fbfb will be in the same strand
        
        //All RCs at residues in the fbfb are compatible with this interval
        compatibleRCs = initializeBitSets(numIntervals, numResAffected, strandRot.rl.getNumAAallowed());

        
        for(int ares=0; ares<numResAffected; ares++){//Loop through affected residues
            //these will have molResNums from startRes to endRes

            int curStrResNum = m.residue[fbfb.startRes+ares].strandResidueNumber;
    ...
    public static DegreeOfFreedom[] makeDOFArray( StrandRotamers strandRot[], int strandMut[][], int mutRes2Strand[],
            int mutRes2StrandMutIndex[], Molecule m, String strandDefault[][], FBFBSettings fs ){

        int numFlexRes = mutRes2Strand.length;
        
        int molToFlexResNumMap[] = new int[m.residue.length];
        Arrays.fill(molToFlexResNumMap,-1);
        ...
        for(int flexResNum=0; flexResNum<numFlexRes; flexResNum++){//Loop through residues
            ...
            molToFlexResNumMap[m.strand[str].residue[strandResNum].moleculeResidueNumber] = flexResNum;
        ...
        for( int str=0; str<m.strand.length; str++ ){
            if(m.strand[str].rotTrans){
                for(int rigidDOFNum=0; rigidDOFNum<6; rigidDOFNum++){
                    DOFList.add( new DegreeOfFreedom(DOFNum, rigidDOFNum, str, strandMut, mutRes2Strand, 
                            mutRes2StrandMutIndex, strandRot[str]) );
                    DOFNum ++;
                }
            }
        }
        ...
        //FBFB parameters
        for(int FBFBNum=0; FBFBNum<m.fbfb.length; FBFBNum++){
            FullBBFlexBlock curBlock = m.fbfb[FBFBNum];
            for(int paramNum=0; paramNum<curBlock.numFreeDOFs; paramNum++){
                DOFList.add( new DegreeOfFreedom(DOFNum,m,FBFBNum,paramNum,fs,molToFlexResNumMap,strandRot) );
                DOFNum++;
            }
        }
    ...
    public double getMeshWidth(){//Get the mesh width to use for this DOF in a FuncLBTerm that depends on it
        ...
        if(type==SCDIHEDRAL)
            ...
            return 0.2;
        else if(type==FBFBPARAM)
            return FBFBMeshWidth;
        else{
