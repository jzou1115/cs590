...
//  ! used for polynomial coefficients
  ...
  TripeptideClosure(Molecule m, int res[], boolean useActualCoord){
      //we can get the lengths, angles, and omegas from either the actualCoordinates or Atom.coord
      init(m,res,useActualCoord);
  }
  ...
  void init(Molecule m, int res[], boolean useActualCoord){
      ...
      for(int a=0;a<3;a++){
          ...
          double NCoordf[], CACoordf[], CCoordf[];
          
          if(useActualCoord){
              NCoordf = m.getActualCoord( m.residue[res[a]].getAtomNameToMolnum("N") );
              CACoordf = m.getActualCoord( m.residue[res[a]].getAtomNameToMolnum("CA") );
              CCoordf = m.getActualCoord( m.residue[res[a]].getAtomNameToMolnum("C") );
          }
          else {
              NCoordf = m.residue[res[a]].getAtomByName("N").coord;
              CACoordf = m.residue[res[a]].getAtomByName("CA").coord;
              CCoordf = m.residue[res[a]].getAtomByName("C").coord;
          }
      ...
      for(int a=0; a<6; a++)
          b_len[a] = rm.norm(vec[a+1]);

      for(int a=0; a<7; a++)
          b_ang[a] = rm.getAngle( rm.scale(vec[a], -1), vec[a+1]);
  ...
  public void initialize_loop_closure(double b_len[], double b_ang[], double t_ang[])
 {
  ...
  double p[] = new double[4], Us[][] = new double[3][3];
