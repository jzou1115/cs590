...
public class ClashFilter {
    ...
    static boolean useEllipses = true;//instead of determining clashes by minimizing the full EPIC energy,
    //just check if there is a DOF vector within the non-clashing ellipses for each voxel
    //since being in an ellipse is necessary but not sufficient for being non-clashing,
    //this method results in a method that errs on the side of non-clashing
    ...
    boolean hasClashes(int level, int[] conf){
        ...
        */
        ...
        if(useEllipses){
            ...
            //DEBUG!!  Will want to inherit initial values
            DoubleMatrix1D ip = new EllipsoidIntersector(lof, null).getFeasibleInitialPoint();
            if(ip==null)
                return true;
            else
                return false;//AND PASS ON AS INIT VAL FOR NEXT LEVEL?
        }
